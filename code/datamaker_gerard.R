...
## Synopsis: Modify Mengyin's code to include other estimation
## procedures and correct some things.
###########################
...
##' Fit the robust regression of cate to the log(counts + 1) data
##'
##' @param Y An ngene by nsamp matrix of observations.
##' @param condition A vector of factors.
##' @param num_sv A positive integer. The number of hidden confounders.
cate_fit_rr <- function(Y, condition, num_sv) {
    cate_rr <- cate::cate(~trt, Y = t(Y),
                          X.data = data.frame(trt = as.numeric(condition) - 1),
                          r = num_sv, fa.method = "ml", adj.method = "rr")
    cate_rr_out <- list()
    cate_rr_out$betahat <- cate_rr$beta
    cate_rr_out$sebetahat <- sqrt(cate_rr$beta.cov.row * cate_rr$beta.cov.col) / sqrt(ncol(Y))
    cate_rr_out$pvalue <- cate_rr$beta.p.value ## median adjusted to correct for standard deviation
                                               ## being biased small
    return(cate_rr_out)
}

##' Fit the negative controls version of cate to the log(counts + 1) data
##'
##' @param Y An ngene by nsamp matrix of observations.
##' @param condition A vector of factors.
##' @param num_sv A positive integer. The number of hidden confounders.
##' @param halfnull A vector of logicals indicating if gene is known to be null (TRUE)
##         or unknown to be null (FALSE).
cate_fit_nc <- function(Y, condition, num_sv, halfnull) {
    cate_nc <- cate::cate(~trt, Y = t(log_counts),
                          X.data = data.frame(trt = as.numeric(condition) - 1),
                          r = num_sv, fa.method = "ml", adj.method = "nc", nc = as.logical(halfnull))
    cate_nc_out <- list()
    cate_nc_out$betahat <- cate_nc$beta
    cate_nc_out$sebetahat <- sqrt(cate_nc$beta.cov.row * cate_nc$beta.cov.col) / sqrt(ncol(Y))
    cate_nc_out$pvalue <- cate_nc$beta.p.value ## median adjusted to correct for standard deviation
                                               ## being biased small
    return(cate_nc_out)
}

##' Fit the soft-thresholding version of LEAPP to the log(counts + 1) data
##'
...
##' @param condition A vector of factors.
##' @param num_sv A positive integer. The number of hidden confounders.
leapp_fit_soft <- function(Y, condition, num_sv) {
    leapp_sparse <- leapp::leapp(dat = Y, pred.prim = as.numeric(condition) - 1,
                                 num.fac = num_sv, method = "soft")
    leapp_sparse_out <- list()
    leapp_sparse_out$betahat <- leapp_sparse$gamma
    leapp_sparse_out$pvalue <- leapp_sparse$p
...
##' Fit the ridge regression version of LEAPP to the log(counts + 1) data
##'
##' Does not return standard errors.
##'
##' @param Y An ngene by nsamp matrix of observations.
##' @param condition A vector of factors.
##' @param num_sv A positive integer. The number of hidden confounders.
leapp_fit_ridge <- function(Y, condition, num_sv) {
    leapp_ridge <- leapp::leapp(dat = Y, pred.prim = as.numeric(condition) - 1,
                              num.fac = num_sv, method = "hard", sparse = FALSE)
    leapp_ridge_out <- list()
    leapp_ridge_out$betahat <- leapp_ridge$gamma
    leapp_ridge_out$pvalue <- leapp_ridge$p
    return(leapp_ridge_out)
}
